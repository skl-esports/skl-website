...
( function( $ ) {
	...
	$( document ).ready( function() {
		...
		progression_megamenu.menu_item_mouseup();
		progression_megamenu.megamenu_status_update();
		//$( '.edit-menu-item-megamenu-status' ).status_update();
		progression_megamenu.update_megamenu_fields();

	});

	// "extending" wpNavMenu
	var progression_megamenu = {

		menu_item_mouseup: function() {
			$( document ).on( 'mouseup', '.menu-item-bar', function( event, ui ) {
				if( ! $( event.target ).is( 'a' )) {
					setTimeout( progression_megamenu.update_megamenu_fields, 300 );
				}
			});
		},
